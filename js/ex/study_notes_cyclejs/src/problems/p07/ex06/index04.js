...
function intent(dom$) {
  ...
  const submit$ = dom$.select('.new_item_submit').events('click')
    .mapTo({type: "NEW_ITEM"})
    //.debug()
  // @change: before ile takeUntil'in benzerini yapmaya çalıştım, fakat her bir type eventini fire etmeye devam ediyor yine de
  //function before(first$) {
    //return (source) => first$.mapTo(source.endWhen(first$)).flatten()
  //}
  function before(first$) {
    return (source) => {
      first$.debug()
      return first$.mapTo(source.last()).flatten()
    }
  }
  const newItem$ = updateInput$.compose(before(submit$))
    .map( (title) => title)
    .debug()
