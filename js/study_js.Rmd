...
## `apply` unboxes array arg and passes them to the fun arg

``` {bash}
node ex/study_js/a05.ts
``` 

The following is equivalent to the next example:
...
## Implement `concatAll` with Examples

``` {bash}
node ex/study_js/a06.ts
``` 
...
## Naming Nested Arrays

Normal way of defining an array:

``` js
as = [ {p1: 1}, {p1: 2} ]
``` 

Using map over an array:

``` js
as.map( a => a.p1 )
``` 

Note that, the name `a` doesn't exist in the definition of `as`

It corresponds to each element of an array.

Let's define the array with `a` elements as named `a'`:

``` js
as = [ a': {p1: 1}, a': {p1: 2} ]
``` 

Now, do it for nested arrays:

``` js
as = [ a': { bs: [ b': {p1: 1} ] } ]
``` 

Note that, we don't specify `a'` and `b'` normally. But they are useful as implicitly defined names, because we have to refer them when mapping over the array `as`. Such as:

``` js
as.map(a => a.bs.map(b => b.p1))
``` 

So, we need 4 names in double-nested array:

``` js
x1 = [ x2': { x3: [ x4': { p1: 1 } ] }]
``` 

`x1` and `x3` are already specified in idiomatic js. `x2` and `x4` are specified only as `map` arguments.

## What does `map` do in nested arrays?

`map` traverses an array and produces another array. 

What about the elements of arrays? Are they protected by `map` operation?

No, `map` protects only arrays and nothing more.

So, if input to `map` is a double-nested array, then the output is a double-nested array too.

Each `map` corresponds to an array. If there are two `map` operations nested, then we have double-nested array.

``` js
as = [ {name, items: [ {id, title} ]} ]
``` 

We want to have the following output:
...
reference:
    ...
    Chapter 9: Monadic Onions
		## Chapter 9: Monadic Onions <url:file:///~/gdrive/mynotes/content/books/fp/book_frisby_adequate_guide_to_functional_programming.md#r=g_10601>
