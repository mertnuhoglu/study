...
## References
		...
		refcard cyclejs <url:file:///~/gdrive/mynotes/content/code/cjs/cjs.md#r=g_10245>
...
## Consuming Actual Data in CycleJS App id=g_10533
...
Generate schema and initial data:
		...
		ref: Implementation of yuml to rdb and yuml to data <url:file:///~/projects/study/r/yuml_to_rdb.Rmd>
...
You can load package by either of these into R:

		devtools::load_all()
		library(yumltordbschema)

		ref: devtools for package building <url:file:///~/gdrive/mynotes/content/code/cr/cr.md#r=sr_0007>
...
``` vim
  #> ~/projects/itr/itr_documentation/data_model/data/view/ddl.sql
https://www.youtube.com/watch?v=o_743uD0vCE&t=218s
:%s/\<BIGINT\>/INT/
``` 
...
Run `npm start` or `npm run start` or `npm run 01`
...
#### v15.10_04 Type safety for all functions

Made `Reducer` type safe.

Edit `~/projects/study/js/vrp/ex/cyclejs_vrp/ex15/src/10_04/interfaces.ts`
...
Edit `~/projects/study/js/vrp/ex/cyclejs_vrp/ex15/src/10_04/components/app/model.ts`
...
Implement Flow01:
		...
		1. User clicks: Plan Listesi > .seç plan > Verileri Getir
		2. Button click sends plan_id
		3. App queries purchase_orders belonging to this plan_id
		4. App views DetailPlan table with these purchase_orders

Curl tests in `<url:/Users/mertnuhoglu/projects/study/js/vrp/study_vrp.Rmd#tn=003 study_vrp>`

Edit `~/projects/study/js/vrp/ex/cyclejs_vrp/ex15/src/10_05/components/plan_panel/intent.ts`

``` js
    const planDetails$: xs<RequestInfo> = dom$.select('.get_plan_details').events('click')
			.map((e) => e.target.dataset.plan_id)
			.map(plan_id => ({
				url: `http://localhost:8080/rest/plan?select=order_line(purchase_order(purchase_order_id))&plan_id=eq.${plan_id}`,
``` 

Edit `~/projects/study/js/vrp/ex/cyclejs_vrp/ex15/src/10_05/components/plan_panel/model.ts`
...
## Run Project: Backend and Frontend id=g_10532
...
docker-compose up 
subzero dashboard
git checkout vrp_psk01_step03
``` 

		walkthrough vrp_psk01 <url:file:///~/projects/itr/itr_documentation/doc_itr.md#r=g_10551>
		deprecated: Consuming Actual Data in CycleJS App <url:file:///~/projects/study/js/vrp/cyclejs_vrp.Rmd#r=g_10533>
...
Edit `~/projects/study/js/vrp/ex/cyclejs_vrp/ex15/src/10_05/index.ts`
