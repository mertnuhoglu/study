// @description: tüm Sources / Sinks tiplerini tam spesifik hale getir (optional olmasın)
import xs from 'xstream';
import { a, article, br, button, div, form, h3, input, label, li, span, sup, table, tbody, td, th, thead, tr, ul } from '@cycle/dom';
import { style } from 'typestyle/lib';
import { Sinks, Sources, SiHot, Omit } from '../../interfaces';
import 'handsontable'

const styles = style({
});

export default function DetailPanel(sources: Sources) {
  ...
  const sinks: Omit<Sinks, 'onion'> & SiHot = {
    DOM: panel_vdom$,
    Hot: state$
      .map(state => state.purchase_orders)
