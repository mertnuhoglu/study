			...
			generify nse columns with se
				...
				filter
					...
					# opt1 application: treat arguments as variables
						arg = "setosa"
						.dots = list( ~Species == arg )
						filter_( iris, .dots = .dots )
					# opt2: se for key of formula expression
						library("lazyeval")
						arg = "setosa"
						key = "Species"
						.dots = list( interp( ~y == x ,
																	.values = list( y = as.name(key), x = arg ) ))
						filter_( iris, .dots = .dots )
					opt1
						family <- 'Scaridae'
						field <- 'Family'
		...
		duplicates
			duplicated rows in dplyr
				dups8 = dd8 %>%
					...
					filter( n() > 1 )
				dups8b = dd8 %>%
					distinct( entity_name, data_field_name ) 
