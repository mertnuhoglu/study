...
# project_id <- "0-7"
project_id <- "0-24"
url_rdb <- "https://docs.google.com/spreadsheets/d/1HMosZ0_z42TxqezqDF7WAozxdu1TNgTs8CMueB5MZdY/edit#gid=0"
```
## Initial Setup
...
```{r read_rdb_requirement_module}
tsk <- googlesheets4::read_sheet(url_rdb, sheet = "Requirement-Module", na = "NA") %>%
  dplyr::mutate(phase = unlist(phase)) %>%
  dplyr::filter(phase == "1") %>%
...
4. Ve sonra tekrar YT'de ilgili fiedları update edelim.
...
url <- "https://youtrack.layermark.com/api/issues"
query = list(fields = "idReadable,created")
map_yt_rdb_rqt <- lapply(1:3, function(i, tsk_df) {
...
tsk_rqt3 <- tsk_rqt2 %>%
        ...
        dplyr::select(task_id, self_issue_id = issue_id, module_id, predecessor_task_id)
tsk_rqt3
```
...
for (i in 1:nrow(bs_parent)) {
  POST_auth(url_command, body = bs_parent[i, ]$body_parent, encode = "form")
}
```
