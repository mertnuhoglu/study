...
ffmpeg -ss "${start_time}" -to "${end_time}" -i "${original_video}" -c:v libx264 -crf 23 -c:a aac test02.mp4
ffmpeg -ss "${start_time}" -to "${end_time}" -i "${original_video}" -c:v libx264 -crf 23 -c:a aac test02b.mkv

start_time=00:00:05.50
end_time=00:00:07.25
ffmpeg -ss "${start_time}" -to "${end_time}" -i "${original_video}" -c:v libx264 -crf 23 -c:a aac test03.mp4
