...
mkdir -p $OUT_DIR &&
#cp "${clip_name}.srt" $OUT_DIR/marks.txt &&
...
#nvim -s "${DIR}/ConvertMarksTxt2MarksTsv.vim" marks.txt &&
...
sed -i 's/\r\n/\n/g' "${clip_name}.tr.srt" &&
sed -i 's/\r//g' "${clip_name}.tr.srt" &&
#cp "${clip_name}.tr.srt" $OUT_DIR/marks.tr.txt &&
#nvim -s "${DIR}/ConvertMarksTxt2MarksTsvTr.vim" marks.tr.txt &&
