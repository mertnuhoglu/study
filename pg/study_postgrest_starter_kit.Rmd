...
path: ~/projects/study/pg/study_postgrest_starter_kit.Rmd
state: wip
...
3. Edit `db/src/sample_data/data.sql`:

        $ curl -H "Authorization: Bearer $JWT_TOKEN" http://localhost:8080/rest/todos?select=id,todo
        [{"id":1,"todo":"item_1_updated"},{"id":3,"todo":"item_3"},{"id":6,"todo":"item_6"}]

4. Create new table: data/table.sql imported from data/schema.sql

5. Create new view as API: api/views_and_procedures.sql imported from api/schema.sql

6. Create sample data: sample_data/data.sql imported from reset.sql

    delete previous data in reset.sql:

        truncate data.client restart identity cascade;

7. Grant privilege for the new view: authorization/privileges.sql imported from init.sql
