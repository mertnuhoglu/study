...
The main entry function for sql generation is `build_sql_for_entity`:

``` r
build_sql_for_entity = function(entity, data_entities, dfl, den) {
  dflf = dfl %>%
...
data_entities = lapply(entities, r_entity, data_model_dir) %>%
	setNames(entities)
data_entities
lapply( entities, build_sql_for_entity, data_entities, dfl, den)
```
...
Step by step, data flow inside `build_sql_for_entity()` is as follows:
...
So, let's update definition of `build_sql_for_entity()` with this `build_sql()` and run the program again:
...
ent_m_cmdtype_m_cmds = lapply( entities, build_sql_for_entity, data_entities, dfl, den) %>%
  setNames(entities)
